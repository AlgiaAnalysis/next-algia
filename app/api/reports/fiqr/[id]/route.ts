...
export async function GET(
  ...
  try {
    ...
    const details = await ReportController.getFIQRReportDetails(reportId)
    ...
    const formattedReport = {
      ...
      score: details.report.par_score,
      status: details.report.par_status,
      clinicalResume: details.report.par_cli_resume,
      observation: details.report.par_observation,
      domains: details.domainReports.map(dr => ({
        domain: dr.pdr_domain,
        score: dr.pdr_score,
        answers: dr.report_answers.map(ra => ({
    ...
    console.error('[GET /api/reports/fiqr/[id]]', error)
    return NextResponse.json(
