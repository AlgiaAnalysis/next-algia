...
export async function POST(request: NextRequest) {
  try {
    ...
    if (session.role !== 'patient') {
      return NextResponse.json(
        { error: 'Apenas pacientes podem responder o FIQR' },
        { status: 403 }
      )
    }
    ...
    // Valida as respostas
    const validation = ReportController.validateFIQRAnswers(answers, questions)
    if (!validation.valid) {
      return NextResponse.json(
    ...
    // Cria o relatorio
    const result = await ReportController.createFIQRReport({
      patientId: BigInt(session.representedAgentId),
      answers,
      questions
    })
    ...
    return NextResponse.json({
      success: true,
      message: 'Questionario FIQR salvo com sucesso',
      scores: result.scores,
      reportId: result.report?.par_id.toString()
    })
  } catch (error) {
    console.error('[POST /api/reports/fiqr]', error)
    return NextResponse.json(
