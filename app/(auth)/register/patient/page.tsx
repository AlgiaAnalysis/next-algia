...
type FormSection = 'personal' | 'credentials' | 'clinical'
...
export default function RegisterPage() {
  ...
  const validateSection = (section: FormSection): boolean => {
    ...
    if (section === 'clinical') {
      if (!formData.gender) {
        newErrors.gender = 'Selecione seu genero'
      }
      // diseaseDiscoverDate e opcional
    }
    ...
    try {
      const response = await fetch('/api/patients/register', {
        ...
        body: JSON.stringify({
          ...
          diseaseDiscoverDate: formData.diseaseDiscoverDate || null
        }),
      })
  ...
  const sections: { id: FormSection; label: string; icon: React.ReactNode }[] = [
    ...
    { id: 'clinical', label: 'Dados Clinicos', icon: <HeartPulse className="h-4 w-4" /> }
  ]
      ...
      <Card className="w-full max-w-lg shadow-xl border border-blue-100 bg-white/90 backdrop-blur-sm">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardDescription className="text-blue-600/70">
            Preencha seus dados para comecar a usar o Algia
          </CardDescription>
        </CardHeader>
          ...
          <CardContent className="space-y-4">
            ...
            {currentSection === 'clinical' && (
              <div className="space-y-4">
                ...
                {/* Genero */}
                <div className="space-y-2">
                  <label htmlFor="gender" className="text-sm font-medium text-blue-900">
                    Genero
                  </label>
                  <div className="relative">
                    ...
                    <select
                      ...
                      className={cn(
                        'flex h-9 w-full rounded-md border bg-transparent px-3 py-1 pl-10 text-base shadow-xs transition-colors',
                        'border-blue-200 focus:border-blue-400 focus:outline-none focus:ring-1 focus:ring-blue-400',
                ...
                {/* Data de descoberta da doenca */}
                <div className="space-y-2 mb-6">
                  <label htmlFor="diseaseDiscoverDate" className="text-sm font-medium text-blue-900">
                    Data de Descoberta da Condicao
                    <span className="text-blue-400 text-xs ml-1">(opcional)</span>
                  </label>
                  <div className="relative">
                    <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-blue-400" />
                    <Input
                      id="diseaseDiscoverDate"
                      type="date"
                      value={formData.diseaseDiscoverDate}
                      onChange={e => handleChange('diseaseDiscoverDate', e.target.value)}
                      className="pl-10 border-blue-200 focus:border-blue-400 focus-visible:ring-blue-400"
                      disabled={isLoading}
                    />
                  </div>
                  <p className="text-xs text-blue-400">
                    Quando voce recebeu o diagnostico de fibromialgia ou condicao similar
                  </p>
                </div>
              </div>
            )}
